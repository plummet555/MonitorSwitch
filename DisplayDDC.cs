...
namespace MonitorSwitchService
{
    class DisplayDDC
    {
        #region Members
        ...
        private static List<DisplayDDC.Physical_Monitor[]> physicalMonitors = new List<Physical_Monitor[]>();
        ...
        public static void Init()
        {
            ...
            bool result = EnumDisplayMonitors(IntPtr.Zero, IntPtr.Zero, enumMonitorsDelegate, IntPtr.Zero);
            Debug.WriteLine("EnumDisplayMonitors: " + result); 
        }
        ...
        public static bool SetDisplaySource(int monitorIndex, int source, out int error)
        {
            ...
            foreach (Physical_Monitor[] monitorArray in physicalMonitors)
            {
                foreach (Physical_Monitor monitor in monitorArray)
                {
                    if (currentMonitorIndex == monitorIndex)
                    {                    
                        bool success = SetVCPFeature(monitor.hPhysicalMonitor, (byte)VCPCodes.InputSource, source);
                        if (!success)
                        {
        ...
        private static bool EnumMonitorsInit(IntPtr hMonitor, IntPtr hdcMonitor, ref DisplayDDC.Rect lprcMonitor, IntPtr dwData)
        {
            ...
            if (success)
            {
                ...
                Debug.WriteLine("Added physical monitor");
            }
        ...
        private static bool GetPhysicalMonitorsFromHMONITOR(IntPtr hMonitor, out Physical_Monitor[] physicalMonitors)
        {
            ...
            Debug.WriteLine("GetPhysicalMonitorsFromHMONITOR: " + fetchSuccess);
            ...
            if (fetchSuccess)
            {
                ...
                {
                    Physical_Monitor? thisMonitor = (Physical_Monitor?)Marshal.PtrToStructure(pUnmanagedMonitorElement, typeof(Physical_Monitor));
                    if (thisMonitor != null)
                    {
                        physicalMonitors[monitorIndex++] = (Physical_Monitor)thisMonitor;
                    }
                    pUnmanagedMonitorElement += sizeofPhysicalMonitor;
                }
            }
